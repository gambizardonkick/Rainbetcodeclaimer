...
class FirebaseDB {
  ...
  async createUser(userData) {
    ...
    const newUser = {
      ...
      rainbetUsername: userData.rainbetUsername || null,
  ...
  async createRainbetAccount(accountData) {
    const accountsRef = this.db.ref('rainbetAccounts');
    const accountId = await this.getNextId('rainbetAccounts');
    
    const newAccount = {
      userId: accountData.userId,
      username: accountData.username ? accountData.username.toUpperCase() : null,
      publicId: accountData.publicId || null,
      telegramChatId: accountData.telegramChatId || null,
  ...
  async findRainbetAccountsByUsername(username) {
    const accountsRef = this.db.ref('rainbetAccounts');
  ...
  async findRainbetAccountsByPublicId(publicId) {
    const accountsRef = this.db.ref('rainbetAccounts');
    const snapshot = await accountsRef.orderByChild('publicId').equalTo(publicId).once('value');
  ...
  async findRainbetAccountsByUserId(userId) {
    const accountsRef = this.db.ref('rainbetAccounts');
  ...
  async findRainbetAccountById(accountId) {
    const snapshot = await this.db.ref(`rainbetAccounts/${accountId}`).once('value');
  ...
  async updateRainbetAccount(accountId, updates) {
    updates.updatedAt = new Date().toISOString();
    await this.db.ref(`rainbetAccounts/${accountId}`).update(updates);
  ...
  async createAuthSession(sessionData) {
    ...
    const newSession = {
      ...
      rainbetAccountId: sessionData.rainbetAccountId,
  ...
  async createTrialHistory(historyData) {
    ...
    const newHistory = {
      ...
      username: historyData.username.toUpperCase(),
  ...
  async deleteExpiredRainbetAccounts() {
    const accountsRef = this.db.ref('rainbetAccounts');
